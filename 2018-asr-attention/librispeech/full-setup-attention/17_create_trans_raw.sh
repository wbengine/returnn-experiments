...
for prefix in dev-clean dev-other test-clean test-other train-clean train-other; do
    ...
    ${returnndir}/tools/dump-dataset-raw-strings.py --dataset "{'class':'LibriSpeechCorpus', 'bpe':{'bpe_file':'data/dataset/trans.bpe.codes', 'vocab_file':'data/dataset/trans.bpe.vocab'}, 'path':'data/dataset', 'audio':{}, 'prefix': '$prefix', 'use_zip': True}" --out data/dataset/$prefix.trans.raw
